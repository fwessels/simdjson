...
ParsedJson::ParsedJson()
    : structurals(nullptr), tape(nullptr),
...
ParsedJson::ParsedJson(ParsedJson &&p)
    : byte_capacity(p.byte_capacity), depth_capacity(p.depth_capacity),
      ...
      current_loc(p.current_loc), structurals(p.structurals),
      tape(p.tape), containing_scope_offset(p.containing_scope_offset),
      ret_address(p.ret_address), string_buf(p.string_buf),
      current_string_buf_loc(p.current_string_buf_loc), valid(p.valid) {
  p.structurals = nullptr;
  p.tape = nullptr;
...
bool ParsedJson::allocate_capacity(size_t len, size_t max_depth) {
  ...
  size_t num_structurals = (ROUNDUP_N(len, 64)/64);
  structurals = new (std::nothrow) uint64_t[num_structurals];
