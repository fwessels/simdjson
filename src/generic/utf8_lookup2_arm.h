...
struct utf8_checker {
  ...
  really_inline void set_overflow(simd8<uint8_t> input) {
    ...
    this->prev_incomplete = input.saturating_sub(max_value);
  }
  ...
  //
  really_inline void check_multibyte_lengths(simd8<uint8_t> input, simd8<uint8_t> prev_input, simd8<uint8_t> prev1) {
    ...
    simd8<uint8_t> is_second_byte(prev1 >= uint8_t(0b11000000u));
    simd8<uint8_t> is_third_byte (prev2 >= uint8_t(0b11100000u));
    simd8<uint8_t> is_fourth_byte(prev3 >= uint8_t(0b11110000u));
  ...
  //
  really_inline void check_special_cases(const simd8<uint8_t> input, const simd8<uint8_t> prev1) {
    //
  ...
  really_inline void check_utf8_bytes(const simd8<uint8_t> input, const simd8<uint8_t> prev_input) {
    ...
    this->check_special_cases(input, prev1);
    this->check_multibyte_lengths(input, prev_input, prev1);
  }
  ...
  really_inline void check_next_input(simd8x64<uint8_t> input) {
    ...
    if (likely(!bits.any_bits_set_anywhere(0b10000000u))) {
      ...
      this->set_overflow(input.chunks[simd8x64<uint8_t>::NUM_CHUNKS-1]);
    }
  }
